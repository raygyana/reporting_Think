    ...
    <ul class="nav" [ngClass]="classMap" (click)="$event.preventDefault()">
        <li *ngFor="let tabz of tabs" [ngClass]="['nav-item', tabz.customClass || '']"
          ...
          <a href="javascript:void(0);" class="nav-link"
            [class.active]="tabz.active" [class.disabled]="tabz.disabled"
            (click)="tabz.active = true" *ngIf="!tabz.hideTab">
            <span [ngTransclude]="tabz.headingRef">{{tabz.heading}}</span>
            <span *ngIf="tabz.removable">
              <span (click)="$event.preventDefault(); removeTab(tabz);" class="fa fa-times-circle-o"></span>
            </span>
            <span *ngIf="tabz.hidable">
              <span (click)="$event.preventDefault(); onHideTab(tabz);" class="fa fa-times-circle-o"></span>
            </span>
          </a>
        </li>
    </ul>
...
export class TabsetComponent implements OnDestroy {
  ...
  public onHideTab(tab: TabDirective, options = { reselect: true, emit: true }): void {
    console.log(" On Hide Tab");
    let index = this.tabs.indexOf(tab);
    if (index === -1 || this.isDestroyed) {
      return;
    }
